...
export default function EnvironmentCard({
  ...
  return (
    <div className="bg-card text-card-foreground border border-border rounded-xl p-5 w-full flex flex-col gap-4">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 bg-success/10 text-success text-xs font-medium px-2.5 py-1 rounded-full w-fit">
          <span className={`w-1.5 h-1.5 rounded-full inline-block ${status === "active" ? "bg-green-500" : "bg-red-500"}`} />
          {status}
        </div>
        {showMenu && (
